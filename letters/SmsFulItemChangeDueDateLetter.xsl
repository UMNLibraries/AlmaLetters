...
<xsl:if test="notification_data/message='DUE_DATE_CHANGE_FOLLOWED_BY_USER_EXPIRY_DATE_CHANGE'">@@user_expiry_date_change_message@@<xsl:for-each select="notification_data/item_loans/item_loan">
@@title@@: <xsl:value-of select="title" />
@@new_due_date@@: <xsl:value-of select="new_due_date_str" />
</xsl:for-each>
</xsl:if>
